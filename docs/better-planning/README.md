...
| `features/harness/harness-prd.md` | prd | **settled** 2026-06-07, **amended 2026-06-09 ×2** — rounds 1+2 folded in; steel-thread scoping (#24), #20–23 closed, worked examples (§4.10); Cloudflare review added coordinator judge pass (#25), risk classifier (#26), pre-filtering/failback (#27); soundness review opened `PhaseId` (#28), constrained the coordinator (#29–#31), made risk rules per-phase (#32), put stripped paths in the report (#33) |
...
| `features/harness/harness-tasks.md` | tasks | **ready** 2026-06-08, **amended 2026-06-09 ×2** — 29 tasks (T1–T29), one PR per milestone; exported to GitHub issues #6–#31 + T27–T29, label `auto-tasks`; soundness amendments folded into T2/T11/T12/T24/T27–T29; **M1 done** (T1–T6, PR #38) + **M2 done** (T7–T11, steel thread closed 2026-06-09); next: build M3 (T12, #17) |
...
| `features/code-review/code-review-prd.md` | prd | **draft 2026-06-19** — Phase 3 feature PRD (+ overview.html). Composite specialists → agreement-based verify → constrained coordinator; eval-harness-first build order. Settled: agreement confidence, net-vs-base scoping, preexisting tier, downstream gating, eval-first. Open: specialist set, N-voter harness amendment, verify cost, coverage↔test-quality boundary — in review |
