...
| `features/code-review/code-review-plan.md` | plan | **settled 2026-06-22** — 6 milestones (M1 verify → M2 contract → M3 eval scaffold → M4 thin slice → M5 panel+tune → M6 risk/config/gating), capability-first (reconciles PRD #5↔#12, PRD amended). Each milestone "run X see Y" verifiable + test-first. Hardened by **two adversarial cold-reader passes** against the real `src/` (pass 1: "not buildable", pass 2: M1 executable) — 7 blockers + 8 should-fixes folded in (PL·4/PL·5). |
