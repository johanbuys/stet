...
| `features/code-review/code-review-rubric-draft.md` | prd (pre-work) | **draft 2026-06-18** — concrete starting rubrics (shared preamble + 4 specialists + coordinator) adapted from open artifacts; to be settled against the eval suite. Architecture validated by pressure test + `research/code-review-best-practices.md`. Next: write the `code-review` PRD (eval-harness-first build order) |
