...
| `features/code-review/code-review-drift.md` | comprehend | **active — M1–M4 synced 2026-06-24** (first sync, full catch-up). **M1–M4 built & merged** (PRs #85–#88; verify, contract, eval scaffold, thin bugs slice); build green. TDD now living (A·3a added). Of 3 open fixes, **2 closed same day:** D6 `composite.run()` refactor (PR #93, 380→165 lines), D4 path-mismatch test (PR #94); **D5 combined-diff gap deferred** (likely unreachable in v1). **M5 (full panel + tune) and M6 (risk/config/gating) not started.** |
