...
| `features/code-review/code-review-tdd.md` | design | **settled 2026-06-22** — TDD (+ overview.html). Area A (agreement-verify stage) live-walked on canvas, areas B–G drafted then accepted in one review round. Net-new `verify.ts` capability; `Finding` contract evolution (`meta.selfConfidence`, deterministic `meta.preexisting`); eval tiered split (cassettes + live). Spikes: cassette record/replay, embedding-match lib. Amended via plan cold-read (B·2 hunk-parse correction). |
| `features/code-review/code-review-plan.md` | plan | **settled 2026-06-22 · revised 2026-06-26 (M6 boundary)** — 6 milestones (M1 verify → … → M6 risk/config/gating), capability-first; two adversarial cold-reader passes (PL·4/PL·5). **M1–M6 built & merged.** Revision pass added **M6.5** (config wiring: T21 `maxFindings` firm, T23 per-specialist `model` provisional) + a Carry-forward list — `verify.voters`→design, **CF-1 review→routing** (T23's prerequisite; review still on the `PI_TEST_MODEL` stopgap), D5 accepted as debt, `risk.thresholds` cut (PL·6). Next: tasks for M6.5. |
| `features/code-review/code-review-tasks.md` | tasks | **M1–M6 done · M6.5 ready 2026-06-26** — 20 tasks (T1–T20) across 6 milestone PRs built & merged (issues #65–#84, milestones #11–#16). **M6.5 (config wiring) appended** by the M6-boundary revision pass: **T21 `maxFindings`** (firm) + **T23 per-specialist `model`** (provisional, blocked on CF-1 review→routing/M7). `verify.voters`→design, D5 debt, `risk.thresholds` cut. Exported: issues #103 (T21) · #104 (T23), Milestone #17. |
| `features/code-review/code-review-drift.md` | comprehend | **active — M5–M6 synced 2026-06-26** (full catch-up). **M1–M6 built & merged** (M5 panel #96/#97, M6 risk/config/gating #98 + config-ignored advisory #99); build green (1159 tests). 6 moves reconciled into the living TDD (E′ fullDiff-classify rule, F config-wiring table, D6 marked resolved). **3 config keys deferred → M6.5** (D7–D9: `maxFindings`, `verify.voters` [needs lens-design call], per-specialist `model`); `risk.thresholds` cut; D5 combined-diff still open. Next: plan revision pass → form **M6.5** → break into tasks. |
