...
| `features/code-review/code-review-drift.md` | comprehend | **active — M1–M4 synced 2026-06-24** (first sync, full catch-up). **M1–M4 built & merged** (PRs #85–#88; verify, contract, eval scaffold, thin bugs slice); build green (1031 tests). TDD now living (A·3a added). **3 open fixes:** D6 decide `composite.run()` refactor **before M5** (human-flagged); D4 path-mismatch test; D5 combined-diff gap (deferred). **M5 (full panel + tune) and M6 (risk/config/gating) not started.** |
